...
termsngifs = db.Table('termsngifs',db.Column('term_id',db.Integer, db.ForeignKey("terms.id")),db.Column('gif_id',db.Integer, db.ForeignKey("gifs.id")))
gifsncollects = db.Table('gifsncollects',db.Column('collection_id',db.Integer, db.ForeignKey("collections.id")),db.Column('gif_id',db.Integer, db.ForeignKey("gifs.id")))
...
class Gif(db.Model):
	...
	def __repr__(self):
		return "{} @ {}".format(self.title,self.emdedURL)
...
class PersonalGifCollection(db.Model):
	...
	gifs = db.relationship('Gif',secondary='gifsncollects',backref=db.backref('collections',lazy='dynamic'),lazy='dynamic')
...
class SearchTerm(db.Model):
	...
	gifs = db.relationship("Gif",secondary='termsngifs',backref=db.backref('terms',lazy='dynamic'),lazy='dynamic')
...
def get_or_create_search_term(term):
	...
	if searchterm:
		...
		for gif in gifs:
			title = gif['title']
			url = gif['embed_url']
			print("TESTING GIFCREATOR")
			print(title)
			print(url)
			g = get_or_create_gif(title=title, url=url)
			term.gifs.append(g)
...
def get_or_create_collection(name, current_user, gif_list=[]):
	...
	collection = db.session.query(PersonalGifCollection).filter_by(name=name, user_id=current_user.id).first()
	if collection:
		return collection
	else:
...
def search_results(search_term):
	...
	all_gifs=[]
	if term:
		all_gifs = term.gifs.all()
	return render_template('searched_gifs.html',gifs=all_gifs,term=term)
...
def create_collection():
	...
	giflist = []
	if form.validate_on_submit():
		for thing in form.gif_picks.data:
			gif = get_gif_by_id(thing)
			giflist.append(gif)
		get_or_create_collection(form.name.data, current_user, giflist)
...
@login_required
def collections():
	mycollections = PersonalGifCollection.query.filter_by(user_id=current_user.id).all()
	print(mycollections)
	return render_template('collections.html', collections=mycollections)
